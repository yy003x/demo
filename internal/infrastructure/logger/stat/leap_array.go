...
func calculateStartTime(now uint64, bucketLengthInMs uint32) uint64 {
	//获取格子的开始时间
	//当前格子开始时间= 当前时间 - 格式已经开始的时间 1 ～ 500ms
...
func NewAtomicBucketWrapArrayWithTime(len int, bucketLengthInMs uint32, now uint64, generator BucketGenerator) *AtomicBucketWrapArray {
	...
	idx := int((now / uint64(bucketLengthInMs)) % uint64(len)) //当前窗口数
	startTime := calculateStartTime(now, bucketLengthInMs)     //当前窗口开始时间
...
func NewAtomicBucketWrapArray(len int, bucketLengthInMs uint32, generator BucketGenerator) *AtomicBucketWrapArray {
	return NewAtomicBucketWrapArrayWithTime(len, bucketLengthInMs, CurrentTimeMillis(), generator)
}

func (aa *AtomicBucketWrapArray) elementOffset(idx int) (unsafe.Pointer, bool) {
	if idx >= aa.length || idx < 0 {
		log.Error(errors.New("array index out of bounds"),
...
func NewLeapArray(sampleCount uint32, intervalInMs uint32, generator BucketGenerator) (*LeapArray, error) {
	...
	return &LeapArray{
		bucketLengthInMs: bucketLengthInMs, //每个格子的时间
		sampleCount:      sampleCount,      //统计的格子数
		intervalInMs:     intervalInMs,     //统计的时间窗口长度
...
func (la *LeapArray) CurrentBucket(bg BucketGenerator) (*BucketWrap, error) {
	return la.currentBucketOfTime(CurrentTimeMillis(), bg)
}
...
func (la *LeapArray) currentBucketOfTime(now uint64, bg BucketGenerator) (*BucketWrap, error) {
	...
	for { //spin to get the current BucketWrap
		...
		if old == nil { //初始化格子数据
			...
			return old, nil
		} else if bucketStart > atomic.LoadUint64(&old.BucketStart) { //当前计算的时间 > 原格子记录的开始时间 (ps:因为是环形, 可能取到之前的周期旧数据 需要清理)
		...
		} else if bucketStart < atomic.LoadUint64(&old.BucketStart) { //按道理应该没有小于,异常状态
			if la.sampleCount == 1 {
...
func (la *LeapArray) Values() []*BucketWrap {
	return la.valuesWithTime(CurrentTimeMillis())
}
...
func (la *LeapArray) ValuesConditional(now uint64, predicate TimePredicate) []*BucketWrap {
	if now <= 0 {
		return make([]*BucketWrap, 0)
	}
	ret := make([]*BucketWrap, 0, la.array.length)
